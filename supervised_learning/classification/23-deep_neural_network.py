...
Deep neural network for binary classification with forward propagation,
cost, evaluation, gradient descent, and upgraded training (verbose/graph).
"""
...
class DeepNeuralNetwork:
    ...
    def train(
        ...
        Args:
            ...
            graph (bool): If True, plot cost every step iterations.
            step (int): Step size for verbose/graph output.
        ...
        Raises:
            ...
            TypeError: If step is not an integer (when needed).
            ValueError: If step is not positive and <= iterations (when needed).
        ...
        for i in range(iterations + 1):
            ...
            if verbose or graph:
                if i % step == 0 or i == iterations:
                    c = self.cost(Y, a_last)
                    points.append(i)
                    costs.append(c)
                    if verbose:
                        msg = "Cost after {} iterations: {}"
                        print(msg.format(i, c))
